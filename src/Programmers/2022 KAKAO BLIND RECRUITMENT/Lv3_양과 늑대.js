/*
    ...
    예를 들어, 위 그림의 경우(루트 노드에는 항상 양이 있습니다) 0번 노드(루트 노드)에서 출발하면 양을 한마리 모을 수 있습니다. 다음으로 1번 노드로 이동하면 당신이 모은 양은 두 마리가 됩니다. 
    이때, 바로 4번 노드로 이동하면 늑대 한 마리가 당신을 따라오게 됩니다. 아직은 양 2마리, 늑대 1마리로 양이 잡아먹히지 않지만, 이후에 갈 수 있는 아직 방문하지 않은 모든 노드(2, 3, 6, 8번)에는 늑대가 있습니다. 
    이어서 늑대가 있는 노드로 이동한다면(예를 들어 바로 6번 노드로 이동한다면) 양 2마리, 늑대 2마리가 되어 양이 모두 잡아먹힙니다. 여기서는 0번, 1번 노드를 방문하여 양을 2마리 모은 후, 8번 노드로 이동한 후(양 2마리 늑대 1마리) 이어서 7번, 9번 노드를 방문하면 양 4마리 늑대 1마리가 됩니다. 
...
function solution(info, edges) {
    ...
    queue.push([link[0], 1, 0]);
    
    while(queue.length) {
        var [nextNode, lamb, wolf] = queue.shift();
        nextNode.forEach((v, i) => {
            var temp = nextNode.slice(0, i).concat(nextNode.slice(i+1));
            switch(info[v]) {
                case LAMB:
                    queue.push([[...temp, ...link[v]], lamb+1, wolf]);
                    answer = Math.max(answer, lamb+1);
                    break;
                case WOLF:
                    if(lamb>wolf+1) {
                        queue.push([[...temp, ...link[v]], lamb, wolf+1]);
                    }
                    break;
            }
        });
